...
def get_anger_score(valfile, word):
    ...
    if token.text in valfile.index:
        return valfile.loc[token.text, "anger"]
    elif token.lemma_ in valfile.index:
        return valfile.loc[token.lemma_, "anger"]
    else:
        return None
...
def process_csv(valfile_path, input_csv, output_csv):
    ...
    try:
        ...
        with open("harris/harrisspeeches/anger_totalscore.csv", "a") as file:
            file.write(f"{input_csv}, {total_anger_score}\n")
...
"""python3 code/lemmatized_emo.py --input "harris/harrisspeeches/line_split_ID" \
                                   --output "harris/NRC emotion scores/lemma per line" \
                                   --type ".csv"
                                   """
#change "sentence_text" to "line_text" and vice versa, and include "sentence_ID" in columns
"""python3 code/lemmatized_emo.py --input "harris/harrisspeeches/sentence_split_ID" \
                                   --output "harris/NRC emotion scores/lemma per sentence" \
                                   --type ".csv"
                                   """