...
def process_csv(valfile_path, input_csv, output_csv):
    ...
    try:
        ...
        required_columns = {'speech_id', 'line_id', 'line_text'}
        ...
        speech_text = data['line_text'].str.cat(sep=" ")
        ...
        with open("harris/harrisspeeches/vad_totalscore.csv", "a") as file:
            file.write(f"{input_csv}, {total_score[0]}, {total_score[1]}, {total_score[2]}, 0\n")
        
        print(f"Processed {input_csv}: Total VAD Scores = {total_score}")

        # Compute VAD scores per line while keeping identifiers
        data[['valence', 'arousal', 'dominance']] = data['line_text'].apply(
        ...
        output_columns = ['speech_id', 'line_id', 'line_text', 'valence', 'arousal', 'dominance']
...
"""python3 code/vad_analysis.py --input "harris/harrisspeeches/line_split_ID" \
                                   --output "harris/VAD scores/per line" \
                                   --type ".csv"
                                   """